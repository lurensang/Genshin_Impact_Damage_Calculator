...
# 从网页爬取角色倍率数据
def fatch_character_data(web_id):
    ...
    elemental_skill = []
    elemental_burst = []
    all_rates = [normal_attack, elemental_skill, elemental_burst]
    skills_tab_pos = []
    alternate_sprint_flag = False

    soup = BeautifulSoup(html, "html.parser")
    skill_attr_block = soup.find_all('li', class_ = "obc-tmpl__switch-item", attrs={"data-target": "skill.attr"})
    if len(skill_attr_block) == 6:
        skills_tab_pos = [0,1,2]
    elif len(skill_attr_block) == 7:
        skills_tab_pos = [0,1,3]
        alternate_sprint_flag = True
    for skill in skills_tab_pos:      # 符合要求的字符串
        ...
        for seg in range(len(data)-1):
            if not alternate_sprint_flag or skill != 3:
                all_rates[skill].append([])
            else:
                all_rates[skill-1].append([])
            ...
            for lv in range(len(vals)-1):
                ...
                if not alternate_sprint_flag or skill != 3:
                    ...
                    all_rates[skill-1][seg].append(rate_temp_num)
    return all_rates
